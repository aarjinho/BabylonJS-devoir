...
function createScene() {
    ...
    for (let i = 0; i < 50; i++) {
        // Générer des valeurs aléatoires pour la taille, la position et la texture
        const size = Math.random() * 20 + 2; // Taille aléatoire entre 2 et 7
        const x = Math.random() * 1000 - 5; // Position x aléatoire entre -5 et 5
        const z = Math.random() * 1000 - 5; // Position z aléatoire entre -5 et 5
        const texture = "images/rock.png"; // Texture fixe
        ...
        sphere.position.x = x;
        sphere.position.y = size / 6; // Positionner la sphère à moitié dans le sol pour qu'elle repose dessus
        sphere.position.z = z;
        ...
        // Associer un physicImpostor à la sphère
        sphere.physicsImpostor = new BABYLON.PhysicsImpostor(sphere, BABYLON.PhysicsImpostor.SphereImpostor, { mass: size, restitution: 0.9 }, scene);
      }
...
function createLights(scene) {
    ...
    //Créer une lumière directionnelle pour simuler le soleil
    var sun = new BABYLON.DirectionalLight("sun", new BABYLON.Vector3(0, -1, 0), scene);
    sun.diffuse = new BABYLON.Color3(1, 1, 1);
    sun.specular = new BABYLON.Color3(1, 1, 1);
    
    //Créer une lumière d'ambiance pour la nuit
    var nightLight = new BABYLON.HemisphericLight("nightLight", new BABYLON.Vector3(0, 1, 0), scene);
    nightLight.diffuse = new BABYLON.Color3(0.1, 0.1, 0.3);
    nightLight.specular = new BABYLON.Color3(0, 0, 0);

    //Définir la couleur et l'intensité initiale du soleil
    sun.intensity = 1;
    sun.diffuse = new BABYLON.Color3(1, 1, 1);
    ...
    keys1.push({ frame: 900, value: 0.1 });

    //Créer des clés d'animation pour passer de la nuit au jour
    var keys2 = [];
    keys2.push({ frame: 0, value: 0.1 });
    keys2.push({ frame: 900, value: 1 });

    //Ajouter les clés d'animation à l'animation
    animation.setKeys(keys1.concat(keys2));

    //Appliquer l'animation à la lumière du soleil
    sun.animations.push(animation);
    
    //Démarrer l'animation
    scene.beginAnimation(sun, 0, 1800, true);
}
...
function createTank(scene) {
    ...
    new BABYLON.SceneLoader.ImportMesh("","models/tank/","tank.glb", scene,(newMeshes, particleSystems, skeletons) => {
        let model=newMeshes[0];
        model.parent=tank
...
function createHeroDude(scene) {
   // load the Dude 3D animated model
    ...
    BABYLON.SceneLoader.ImportMesh("", "models/source/", "girl.glb", scene,(newMeshes, particleSystems, skeletons) => {
        let girl=newMeshes[0];
        girl.position = new BABYLON.Vector3(0,0,30);
        girl.scaling = new BABYLON.Vector3(8,8,8)
